		...
		while (mRunning)
		{
			if (mCloud) {
				...
				*p = *mCloud;
				...
				cout << "===================" << endl;
				cout << "Point size: " << p->size() << endl;

				vector<float> posx, posy, posz;
				for (int i = 0; i < p->size(); i++)
				{
					posx.push_back(p->points[i].x);
					posy.push_back(p->points[i].y);
					posz.push_back(p->points[i].z);
				}
				//Send OSC
				OSCSender::GetInstance().sendOSCMessage("/x", posx);
				OSCSender::GetInstance().sendOSCMessage("/y", posy);
				OSCSender::GetInstance().sendOSCMessage("/z", posz);
			}
